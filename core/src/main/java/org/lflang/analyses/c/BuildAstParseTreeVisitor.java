...
public class BuildAstParseTreeVisitor extends CBaseVisitor<CAst.AstNode> {
  ...
  public CAst.AstNode visitDeclaration(DeclarationContext ctx) {
    if (ctx.declarationSpecifiers() != null && ctx.initDeclaratorList() != null) {
      ...
      ArrayList<String> supportedTypes =
          new ArrayList<String>(Arrays.asList("int", "long", "double", "_Bool"));
      if (declSpec.typeSpecifier().Int() != null
          || declSpec.typeSpecifier().Long() != null
          || declSpec.typeSpecifier().Double() != null) type = CAst.VariableNode.Type.INT;
      ...
      List<InitDeclaratorContext> initDeclList = ctx.initDeclaratorList().initDeclarator();
      ...
      CAst.AstNode initNode =
          visitAssignmentExpression(initDecl.initializer().assignmentExpression());
  ...
  /**
   * This visit function builds StatementSequenceNode, AssignmentNode, OpaqueNode, IfBlockNode,
   * AdditionNode, SubtractionNode, MultiplicationNode, DivisionNode, EqualNode, NotEqualNode,
   * LessThanNode, GreaterThanNode, LessEqualNode, GreaterEqualNode, SetPortNode,
   * ScheduleActionNode.
   *
   * @param ctx
   * @return
